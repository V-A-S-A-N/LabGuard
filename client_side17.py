...
def get_monitor_status():
    """Retrieve Monitor status using Win32_DesktopMonitor with availability check."""
    status = "Missing"
    pythoncom.CoInitialize()
    try:
        c = wmi.WMI()
        ...
        for monitor in monitors:
            # Check if the monitor is available (Availability == 3 indicates "Running/Full Power")
            if getattr(monitor, "Availability", None) == 3:
                status = "Present"
                break
    except Exception as e:
        logging.error("Error retrieving Monitor status: %s", e)
...
def get_ethernet_status():
    """Retrieve Ethernet connection status for physical adapters."""
    status = "Disconnected"
    pythoncom.CoInitialize()
    try:
        c = wmi.WMI()
        # Query for physical Ethernet adapters only
        adapters = c.Win32_NetworkAdapter(NetConnectionID="Ethernet", PhysicalAdapter=True)
        for adapter in adapters:
            if adapter.NetConnectionStatus == 2:  # 2 indicates "Connected"
                status = "Connected"
                break
    except Exception as e:
        logging.error("Error retrieving Ethernet status: %s", e)
