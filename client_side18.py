...
SYSTEM_ID = 1
SERVER_IP = '192.168.137.1'  # Replace with your server's IP address # 127.0.0.1
SERVER_PORT = 5000
    ...
    while True:
        try:
            ...
            if "Quota violation" in err_str:
                ...
                time.sleep(30)  # wait longer if quota is exceeded
            else:
    ...
    try:
        ...
        for event in usb_event_generator(query):
            try:
                ...
                r = "Device Removal"
                c = "Device Connected"
                event_type = r if(event_type == "Deletion") else c
                anomaly = {
                    'SystemID': SYSTEM_ID,
                    'TimeGenerated': time.strftime("%Y-%m-%d %H:%M:%S"),
...
def get_system_status():
    ...
    try:
        c = wmi.WMI()
        # USB Devices: Use ConfigManagerErrorCode == 0 for operational devices
        ...
        for dev in pnp_devices:
            ...
            if dev_id.startswith("USB") and getattr(dev, "ConfigManagerErrorCode", -1) == 0:
                if "bluetooth" not in dev_name:  # Exclude Bluetooth
                    usb_devices.append(dev_name)
        
        # Summarize USB devices
        ...
        # GPU: Check for operational video controllers
        ...
        # Monitor and infer VGA/HDMI from connected monitors
        monitors = c.Win32_DesktopMonitor()
        for mon in monitors:
            if getattr(mon, "Availability", 0) == 3:  # Running/Full Power
                status["Monitor"] = "Present"
                mon_name = getattr(mon, "Name", "").lower()
                # Heuristic: Assume VGA or HDMI based on monitor name or system config
                if "vga" in mon_name:
                    vga_monitor = True
                elif "hdmi" in mon_name:
                    hdmi_monitor = True
                break  # One active monitor is sufficient
        
        # Fallback to video controller for VGA/HDMI if no monitor name clue
        ...
        for adapter in adapters:
            if getattr(adapter, "NetConnectionStatus", 0) == 2:  # Connected
