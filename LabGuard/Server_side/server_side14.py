...
# Global SSE queue for pushing update events
sse_queue = queue.Queue()

def push_update(update_data):
    """Push an update event to the SSE queue only if at least one value is True.
    Only include keys with True values in the pushed update."""
    filtered = {k: v for k, v in update_data.items() if v}
    if filtered:  # Only push if there's at least one True flag.
        sse_queue.put(filtered)

def log_anomaly_to_csv(timestamp, system_id, device_name, device_id, event, sender_ip):
    """Log anomaly to CSV, console, and push an SSE update."""
    ...
    # Determine which parts need updating based on the event.
    # For example, update both sections on heartbeat; device events update devices.
    if event == "Heartbeat":
        update = {"updateSidebar": True, "updateDevices": True}
    elif event.startswith("Device"):
        update = {"updateSidebar": False, "updateDevices": True}
    elif event.startswith("Anomaly"):
        update = {"updateSidebar": True, "updateDevices": False}
    else :
        update = {"updateSidebar": True, "updateDevices": False}
    push_update(update)
    ...
    def handle(self):
        ...
        try:
            ...
            if "StatusDetails" in anomaly:
                ...
                if event == "Initial Status":
                    push_update({"updateSidebar": False, "updateDevices": True})
                    return
    ...
    while True:
        with last_seen_lock, shutdowned_clients_lock:
            ...
            for system_id in list(last_seen):
                if system_id in shutdowned_clients:
                    continue
...
def index():
    ...
    with last_seen_lock, client_status_lock:
        systems = [
            {"system_id": sid,
             "ip": ip,
             "last_seen": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(lt)),
    ...
    systems.sort(key=lambda x: (x["status"] != "Online",
                                -time.mktime(time.strptime(x["last_seen"], "%Y-%m-%d %H:%M:%S"))))
    
    anomalies = []
    try:
        with open(ANOMALY_CSV_FILENAME, 'r') as f:
            rows = list(csv.DictReader(f))
            # Get the last 20 anomalies in reverse order.
...
# Endpoint to return only the sidebar fragment (recent anomalies)
@app.route("/sidebar")
def sidebar():
    anomalies = []
    try:
        ...
        pass
    return render_template("sidebar_fragment.html", anomalies=anomalies)

# Endpoint to return only the devices (main content)
@app.route("/devices")
def devices():
    current_time = time.time()
    with last_seen_lock, client_status_lock:
    ...
    systems.sort(key=lambda x: (x["status"] != "Online",
                                -time.mktime(time.strptime(x["last_seen"], "%Y-%m-%d %H:%M:%S"))))
    return render_template("devices_fragment.html", systems=systems)

# SSE endpoint to stream update events.
@app.route('/sse')
def sse():
    def event_stream():
        while True:
            update_data = sse_queue.get()
...
def main():
    ...
    for target in (server.serve_forever, heartbeat_check, lambda: app.run(host="0.0.0.0", port=8000, threaded=True)):
